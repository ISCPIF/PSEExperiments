...
def execution(samples: Int) = {
  val explo =
    ExplorationTask (
      (vision in Range(0.0, 32.0).size(samples)) x
      (minimumSeparation in Range(0.0, 32.0).size(samples)) x
      (maxAlignTurn in Range(0.0, math.Pi).size(samples)) x
      (maxCohereTurn in Range(0.0, math.Pi).size(samples)) x
      (maxSeparateTurn in Range(0.0, math.Pi).size(samples))
    )

  val outputCSV = AppendToCSVFileHook(s"./grid${samples}.csv", vision, minimumSeparation, maxAlignTurn, maxCohereTurn, maxSeparateTurn, groupCount, relativeDiffusion, velocity)
